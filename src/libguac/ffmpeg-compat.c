 ...
 */
static int guac_write_packet(guac_video* video, void* data, int size) {
    ...
    while (remaining > 0) {
        ...
        int written = write(STDOUT_FILENO, current, remaining);
        if (written <= 0)
            break;
    ...
    guac_protocol_send_blobs(video->socket, video->stream, data, size);
    return 0;
}

int guac_avcodec_encode_video(guac_video* video, AVFrame* frame) {
...
/* For libavcodec < 54.1.0: packets were handled as raw malloc'd buffers */
    ...
    if (used < 0) {
        /* guac_log(GUAC_LOG_WARNING, "Error encoding frame #%" PRId64,
                video->next_pts); */
        free(data);
        return -1;
    }
    ...
    guac_write_packet(video, data, used);
    free(data);
    return 1;
...
/* For libavcodec < 57.37.100: input/output was not decoupled */
    ...
    if (got_data) {
        guac_write_packet(video, packet.data, packet.size);
    ...
    /* if (!got_data)
        guac_log(GUAC_LOG_DEBUG, "Frame #%08" PRId64 ": queued for later",
                video->next_pts); */
